...
class ReVersionDataset(Dataset):
    ...
    def __init__(
        ...
        ######### Distill #########
        local_f_distill = open(os.path.join(data_root, 'distill_clean_text.json'))
        self.templates_distill = json.load(local_f_distill)
        print(f'self.templates_distill={self.templates_distill}')
    ...
    def __getitem__(self, i):
        ...
        ################## Distill #####################
        clean_text = random.choice(self.templates_distill[image_name]).format(placeholder_string)
        
        example["student_input_ids"] = self.tokenizer(
            text,
            padding="max_length",
            truncation=True,
        ...
        ################## Distill #####################
        example["teacher_input_ids"] = self.tokenizer(
            clean_text,
            padding="max_length",
            truncation=True,
            max_length=self.tokenizer.model_max_length,
...
def main():
    ...
    token_embeds = student_text_encoder.get_input_embeddings().weight.data
    ...
    ############# Distill freeze student ###########
    student_unet.requires_grad_(False) # free unet and use lora attns
    # Freeze all parameters except for the token embeddings in text encoder
    student_text_encoder.text_model.encoder.requires_grad_(False)
    student_text_encoder.text_model.final_layer_norm.requires_grad_(False)
    student_text_encoder.text_model.embeddings.position_embedding.requires_grad_(False)
    
    ############# Distill freeze teacher ###########
    
    teacher_unet.requires_grad_(False)
    teacher_text_encoder.requires_grad_(False)
    
    
    ######### Distill set correct lora layers for student unet ###########
    lora_attn_procs = {}
    for name in student_unet.attn_processors.keys():
        cross_attention_dim = None if name.endswith("attn1.processor") else student_unet.config.cross_attention_dim
        if name.startswith("mid_block"):
            hidden_size = student_unet.config.block_out_channels[-1]
        elif name.startswith("up_blocks"):
            block_id = int(name[len("up_blocks.")])
            hidden_size = list(reversed(student_unet.config.block_out_channels))[block_id]
        elif name.startswith("down_blocks"):
            block_id = int(name[len("down_blocks.")])
            hidden_size = student_unet.config.block_out_channels[block_id]
    ...
    student_unet.set_attn_processor(lora_attn_procs)
    student_lora_layers = AttnProcsLayers(student_unet.attn_processors)
    ######### Distill set correct lora layers ###########
    ...
    ########### Distill Create Optimizer ###########
    # Optimizer creation
    params_to_optimize = list(filter(lambda p: p.requires_grad, student_lora_layers.parameters()))
    params_to_optimize = params_to_optimize + list(filter(lambda p: p.requires_grad, student_text_encoder.get_input_embeddings().parameters()))
    ...
    student_lora_layers, student_text_encoder,  optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        student_lora_layers, student_text_encoder, optimizer, train_dataloader, lr_scheduler)
    ...
    orig_embeds_params = accelerator.unwrap_model(
        student_text_encoder).get_input_embeddings().weight.data.clone()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(student_unet): ############## Distill  ########## 
                ...
                teacher_encoder_hidden_states = teacher_text_encoder(batch["teacher_input_ids"])[0].to(dtype=weight_dtype)
                student_encoder_hidden_states = student_text_encoder(batch["student_input_ids"])[0].to(dtype=weight_dtype)

                # Predict the noise residual
                
                student_model_pred = student_unet(noisy_latents, timesteps, student_encoder_hidden_states).sample
                with torch.no_grad():
                    teacher_model_pred = teacher_unet(noisy_latents, timesteps, teacher_encoder_hidden_states).sample
                ...
                denoise_loss = F.mse_loss(
                    student_model_pred.float(), target.float(), reduction="mean")
                weighted_denoise_loss = args.denoise_loss_weight * denoise_loss
                loss += weighted_denoise_loss

                student_token_embedding = accelerator.unwrap_model(student_text_encoder).get_input_embeddings()  # with grad

                # L_steer
                if args.steer_loss_weight > 0:
                    assert args.num_positives > 0
                    steer_loss = calculate_steer_loss(
                        student_token_embedding,
                ...
                distill_loss = F.mse_loss(teacher_model_pred.float(), student_model_pred.float(), reduction="mean")
                loss += args.distill_loss_weight * distill_loss
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    save_progress(student_text_encoder, placeholder_token_id,
        ...
        if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
            ...
            pipeline = DiffusionPipeline.from_pretrained(
                args.pretrained_model_name_or_path,
                text_encoder=accelerator.unwrap_model(student_text_encoder),
                tokenizer=tokenizer,
                unet=student_unet,
                vae=vae,
                revision=args.revision,
    ...
    if accelerator.is_main_process:
        ...
        save_progress(student_text_encoder, placeholder_token_id, accelerator, args,
                      save_path)
